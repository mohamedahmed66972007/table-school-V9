...
export function EditableClassSchedule({
  ...
  const saveSlotsMutation = useMutation({
    mutationFn: async (slots: { day: Day; period: Period; teacherId: string }[]) => {
  ...
  const handleSlotChange = (day: Day, period: Period, teacherId: string) => {
    const key = `${day}-${period}`;
    setEditedSlots(new Map(editedSlots.set(key, teacherId)));
  };
  ...
  const detectConflicts = (): Conflict[] => {
    ...
    DAYS.forEach((day) => {
      PERIODS.forEach((period) => {
        ...
        // Use allSlots to check across ALL classes, not just the current one
  ...
  const handleSave = () => {
    ...
    // Build the complete schedule by merging existing and edited slots
    const completeSchedule: { day: Day; period: Period; teacherId: string }[] = [];
    ...
    saveSlotsMutation.mutate(completeSchedule);
  };
                    ...
                    return (
                      ...
                      >
                        <Select
                          ...
                          onValueChange={(value) => handleSlotChange(day, period, value === "none" ? "" : value)}
                        >
